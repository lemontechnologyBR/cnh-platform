...
// ─── Foto 3x4 e assinatura (XObjects do cnh_luis.pdf) ────────────────────────
// Foto:       x=74, y=632, 60×80 pt  (Image-7098480789)
// Assinatura: x=79, y=614, 45×16 pt  (Image-2000805986)
...
async function embedImageFromDataUrl(pdfDoc, dataUrl) {
  ...
  if (type === 'png' || type === 'webp') {
    try { return await pdfDoc.embedPng(bytes) } catch { /* webp as png may fail */ }
  }
...
export async function generateCnhPdf(data) {
  ...
  // ── 1. Apaga texto e imagens do template (foto sempre; assinatura se custom) ─
